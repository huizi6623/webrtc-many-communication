...
let FeatTrainer =  (function () {
    ...
    FeatTrainer.prototype.findTransform = function (matches, screenKeyPoints, patternKeyPoints) {
        ...
        let ransac_param = new jsfeat.ransac_params_t(num_model_points, reproj_threshold, 0.5, 0.99);
        let pattern_xy = [];
        let screen_xy = [];
        ...
        for (let i = 0; i < count; ++i) {
            let m = matches[i];
            let s_kp = screenKeyPoints[m.screen_idx];
            let p_kp = patternKeyPoints[m.pattern_lev][m.pattern_idx];
            pattern_xy[i] = { x: p_kp.x, y: p_kp.y };
            screen_xy[i] = { x: s_kp.x, y: s_kp.y };
        }
        // estimate motion
        let homo3x3 = new jsfeat.matrix_t(3, 3, jsfeat.F32C1_t);
        let match_mask = new jsfeat.matrix_t(this.keyPointsPerlevel, 1, jsfeat.U8C1_t);
        let ok = jsfeat.motion_estimator.ransac(ransac_param, mm_kernel, pattern_xy, screen_xy, count, homo3x3, match_mask, 1000);
    ...
    FeatTrainer.prototype.matchPattern = function (screen_descriptors, pattern_descriptors) {
        ...
        let match_threshold = this.matchThreshold;
        ...
        for (qidx = 0; qidx < q_cnt; ++qidx) {
            ...
            for (lev = 0; lev < num_train_levels; ++lev) {
                let lev_descr = pattern_descriptors[lev];
    ...
    FeatTrainer.prototype.describeFeatures = function (img_u8) {
        let img_u8_smooth = this._imgU8Smooth;
        let screen_corners = this._screenCorners;
        ...
        let max_per_level = this.keyPointsPerlevel;
        let num_corners = this.detectKeyPoints(img_u8_smooth, screen_corners, max_per_level);
        let descriptors = new jsfeat.matrix_t(32, max_per_level, jsfeat.U8_t | jsfeat.C1_t);
